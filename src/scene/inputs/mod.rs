...
pub fn camera_orbit_controls(
    mut query: Query<
        (&mut Transform, &mut components::OrbitCamera),
        With<components::Main3DCameraMarker>,
    >,
    ...
    time: Res<Time>,
    mut camera_target: ResMut<resources::CameraTarget>,
) {
    // Constants
    let zoom_speed = 1.0;
    let pan_speed = 0.5;
    ...
    for (mut transform, mut orbit_camera) in query.iter_mut() {
        ...
        if mouse_button_input.pressed(MouseButton::Middle) {
            for event in mouse_motion_events.read() {
                ...
                let delta = event.delta;
                let right =
                    transform.rotation * Vec3::X * -delta.x * pan_speed * time.delta_seconds();
        ...
        camera_target.translation = orbit_camera.target;
    }
}
